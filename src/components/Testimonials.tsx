...
export default function Testimonials() {
  ...
  return (
    <section className="py-16 md:pb-16 md:pt-16 px-6 relative overflow-hidden ">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8 px-4">
          <div className="max-w-2xl space-y-3">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-[11px] md:text-[12px] uppercase tracking-[0.4em] font-medium text-[#1f4d3a]/80 font-sans">
                Patient Stories
              </h2>
            </div>
        ...
        >
          <div className="overflow-hidden">
            <div
              ...
              style={{
                transform: `translateX(-${currentIndex * (100 / (isMobile ? 1.15 : 3))}%)`,
              }}
            >
              {testimonials.map((t, i) => (
                <div
                  key={i}
                  className={`flex-shrink-0 px-3 ${isMobile ? "w-[87%]" : "w-1/3"}`}
                >
                  <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-[#1f4d3a]/5  relative group  flex flex-col h-full min-h-[240px] md:min-h-[280px]">
                    ...
                    <div className="flex flex-col h-full">
                      <div className="flex gap-0.5 mb-4">
                        {[...Array(5)].map((_, star) => (
                          <Star
                            key={star}
                            size={14}
                            className="fill-[#fab244] text-[#fab244]"
                          />
                        ))}
                      </div>
                      ...
                      <div className="pt-6 mt-6 border-t border-gray-50">
                        <h4 className="font-serif text-[#1f4d3a] text-lg font-bold leading-tight">
                          {t.name}
                        </h4>
                        <p className="text-[10px] uppercase tracking-[0.15em] text-[#1f4d3a]/50 font-bold mt-1">
                          {t.role}
                        </p>
                      </div>
                    </div>
