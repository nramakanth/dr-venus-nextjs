...
const navItems = [
        ...
        megaMenu: [
            {
                ...
                items: [
                    ...
                    { name: "Acne Scar Removal", href: "/acne-scar-removal" },
                    { name: "Laser Hair Removal", href: "/laser-hair-removal" },
                    { name: "Tattoo Removal", href: "/tattoo-removal" },
                    { name: "Skin Lightening", href: "/skin-lightening" },
            ...
            },
            {
                ...
                items: [
                    ...
                    { name: "Skin Lifting", href: "/skin-lifting" },
            ...
            },
            {
                ...
                items: [
                    ...
                    { name: "Liposuction", href: "/liposuction" },
                    { name: "Body Sculpting", href: "/body-sculpting" },
                ]
            }
        ]
    },
    { name: "Blog", href: "/blog" },
    {
        ...
        href: "/courses",
        dropdown: [
            { name: "Professional Dermatology", href: "/courses/professional" },
            { name: "Cosmetology Basics", href: "/courses/basics" },
        ]
    },
    {
        name: "Contact Us",
...
export default function Navbar() {
    ...
    return (
        <header className="w-full fixed top-0 z-50 pointer-events-none">
                ...
                <div className="py-2.5 px-6">
                    <div className="max-w-7xl mx-auto flex justify-between items-center text-[11px] md:text-sm font-sans text-primary/80">
                        ...
                        <div className="flex gap-4 md:gap-8 items-center w-full lg:w-auto justify-between lg:justify-end">
                            <a href="mailto:info@drvenus.in" className="flex items-center gap-1.5 hover:text-primary transition-colors duration-300">
                                <Mail size={12} strokeWidth={2.5} />
                                <span className="hidden sm:inline">info@drvenus.in</span>
                            </a>
                            <a href="tel:+917777977027" className="flex items-center gap-1.5 hover:text-primary transition-colors duration-300">
                                <Phone size={12} strokeWidth={2.5} />
                                <span className="font-bold underline underline-offset-2 tracking-tight">+91 77779 77027</span>
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div className="w-full px-4 md:px-6 pointer-events-none flex justify-center">
                <nav className={`transition-all duration-500 pointer-events-auto ${isScrolled ? "max-w-6xl w-full mx-auto mt-4 rounded-3xl md:rounded-full bg-white/90 backdrop-blur-lg shadow-2xl py-3 px-8 md:px-12" : "w-full bg-white py-5 px-6"}`}>
                    <div className={`mx-auto flex justify-between items-center ${isScrolled ? "w-full" : "max-w-7xl"}`}>
                        ...
                        {/* Desktop Navigation */}
                        <div className={`hidden lg:flex items-center gap-6 xl:gap-8 ${isScrolled ? "text-[15px]" : "text-[15px]"} font-medium tracking-wide font-sans`}>
                            {navItems.map((item) => (
                        ...
                        <div className="flex items-center gap-5">
                            <Link href="/appointment-form">
                                <button className="hidden lg:flex bg-[#1f4d3a] text-white px-6 md:px-8 py-2.5 md:py-3.5 rounded-full font-bold text-xs md:text-sm shadow-xl shadow-[#1f4d3a]/20 hover:bg-[#1f4d3a]/90 transition-all duration-300 hover:scale-105 active:scale-95 border border-[#1f4d3a] whitespace-nowrap">
                                    {isScrolled ? "Book Now" : "Book Appointment"}
                                </button>
                            </Link>

                            {/* Mobile Menu Toggle */}
                            <button className="lg:hidden text-[#1f4d3a] p-2 hover:bg-gray-50 rounded-xl transition-colors" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
            ...
            {/* Mobile Menu */}
            {isMobileMenuOpen && (
                <div className={`lg:hidden fixed left-0 right-0 bottom-0 bg-white z-40 px-6 pt-2 pb-20 flex flex-col animate-in slide-in-from-top duration-300 pointer-events-auto overflow-y-auto ${isScrolled ? "top-[70px]" : "top-[100px]"}`}>
                    <div className="flex flex-col">
                        {navItems.map((item) => (
                            <div key={item.name} className="flex flex-col border-b border-gray-50 last:border-0">
                                {item.megaMenu ? (
                                    <>
                                        ...
                                        >
                                            {item.name} <ChevronDown size={16} className={`${activeDropdown === item.name ? "rotate-180" : ""} transition-transform text-gray-400`} />
                                        </button>
                                        <div className={`flex flex-col gap-6 pl-4 transition-all duration-500 ease-in-out ${activeDropdown === item.name ? "max-h-[2000px] opacity-100 py-4 pb-8" : "max-h-0 opacity-0 overflow-hidden"}`}>
                                            {item.megaMenu.map((cat) => (
                                    ...
                                    <>
                                        ...
                                        <div className={`flex flex-col gap-3 pl-4 overflow-hidden transition-all duration-300 ${activeDropdown === item.name ? "max-h-96 py-2 pb-4" : "max-h-0"}`}>
                                            {item.dropdown.map((subItem) => (
                                                <Link
                                                    key={subItem.name}
                                                    href={subItem.href}
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-50">
                        <Link href="/appointment-form" onClick={() => setIsMobileMenuOpen(false)}>
                            <button className="w-full bg-[#1f4d3a] text-white py-3.5 rounded-xl font-bold text-sm shadow-lg shadow-[#1f4d3a]/10">
                                Book Appointment
                            </button>
                        </Link>
                    </div>
